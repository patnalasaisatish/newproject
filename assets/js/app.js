(function ($) {
  ...
  //accordion
  if ($(".findox-accordion").length) {
    var accordionGrp = $(".findox-accordion");
        ...
        $(this)
          ...
          .on("click", function () {
            if ($(this).parent().hasClass("active") === false) {
              $(".findox-accordion." + accordionName)
                .find(".accordion")
                .removeClass("active");
              $(".findox-accordion." + accordionName)
                .find(".accordion")
  ...
  function thmOwlInit() {
    // owl slider
    let findoxowlCarousel = $(".findox-owl__carousel");
    if (findoxowlCarousel.length) {
      findoxowlCarousel.each(function () {
        let elm = $(this);
    ...
    let findoxowlCarouselNav = $(".findox-owl__carousel--custom-nav");
    if (findoxowlCarouselNav.length) {
      findoxowlCarouselNav.each(function () {
        let elm = $(this);
    ...
    let findoxowlCarouselWithFilter = $(".findox-owl__carousel--filter");
    if (findoxowlCarouselWithFilter.length) {
      findoxowlCarouselWithFilter.each(function () {
        let elm = $(this);
        let options = elm.data("owl-options");
        ...
        $(filtersDiv).on("click", ".item", function () {
          ...
          setTimeout(function () {
            ...
            elm
              .find(".findox-owl__carousel__counter__current")
    ...
    let findoxowlCarouselWithCounter = $(".findox-owl__carousel--with-counter");
    if (findoxowlCarouselWithCounter.length) {
      findoxowlCarouselWithCounter.each(function () {
        let elm = $(this);
        let options = elm.data("owl-options");
        ...
        elm
          .on("initialized.owl.carousel", function (event) {
            ...
            elm.append(
              '<div class="findox-owl__carousel__counter"><span class="findox-owl__carousel__counter__current"></span> <span class="findox-owl__carousel__counter__total"></span></div>'
            );
            elm
              .find(".findox-owl__carousel__counter__current")
              .text(carousel.relative(carousel.current()) + 1);
            elm
              .find(".findox-owl__carousel__counter__total")
              .text(carouselCount);
          })
          .owlCarousel(
            "object" === typeof options ? options : JSON.parse(options)
  ...
  function findoxSlickInit() {
    // slick slider
    let findoxslickCarousel = $(".findox-slick__carousel");
    if (findoxslickCarousel.length) {
      findoxslickCarousel.each(function () {
        let elm = $(this);
        let options = elm.data("slick-options");
        let findoxslickCarousel = elm.slick(
          "object" === typeof options ? options : JSON.parse(options)
        );
      });
    }
    let findoxslickCarouselCounter = $(".findox-slick__custome-counter");
    if (findoxslickCarouselCounter.length) {
      findoxslickCarouselCounter.each(function () {
        let elm = $(this);
        let options = elm.data("slick-options");
        let currentSlide;
        let slidesCount;
        let sliderCounter = document.createElement("div");
        sliderCounter.classList.add("findox-slick__counter");

        let updateSliderCounter = function (slick, currentIndex) {
          currentSlide = slick.slickCurrentSlide() + 1;
          slidesCount = slick.slideCount;
          $(sliderCounter).html(
            '<span class="findox-slick__counter__active">' +
              currentSlide +
              "</span>" +
              "" +
              "<span>" +
  ...
  $(".findox-owl__carousel").on("initialized.owl.carousel", function () {
  ...
  //Strech Column
  function findox_stretch() {
    var i = $(window).width();
    $(".row .findox-stretch-element-inside-column").each(function () {
      var $this = $(this),
  ...
  function findoxChart() {
    const chartElements = document.querySelectorAll(".findox-chart");
    ...
    chartElements.forEach(function (self) {
      ...
      try {
        ...
        gradient.addColorStop(0, "rgba(0, 102, 84, 0.4)");
        gradient.addColorStop(1, "rgba(0, 102, 84, 0)");
        if (
  ...
  document.addEventListener("DOMContentLoaded", findoxChart);
